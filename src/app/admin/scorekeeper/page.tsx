// src/app/admin/scorekeeper/page.tsx
...
export default function ScorekeeperOverviewPage() {
  ...
  const locations = data?.divisions
    .map((d) => d.location)
    .filter((l, idx, arr) => l && arr.findIndex((x) => x?._id === l._id) === idx) || [];
    ...
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardContent className="p-4">
          ...
          {/* Reset Button */}
          {(locationFilter !== "all" || divisionFilter !== "all" || dateFilter) && (
            <div className="mt-4">
              <Button
                variant="outline"
                size="sm"
...
function DivisionSection({ division }: { division: OverviewData["divisions"][0] }) {
...
// Game Card Component
function GameCard({ game }: { game: OverviewData["divisions"][0]["games"][0] }) {
  const isCompleted = game.status;
  const isStarted = game.started;

  return (
    <Link href={`/admin/scorekeeper/${game._id}`}>
